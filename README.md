...
移动端的语音输入功能使用阿里云百炼 Paraformer 实时语音转写模型，与你使用的 Qwen 大模型共享同一个 API Key。

语音输入无需额外配置。只要后台管理页面中 AI 设置使用的是阿里云百炼（DashScope）平台的 API Key，登录移动端后即可直接使用语音按钮。服务端会自动代理 WebSocket 连接到 DashScope，前端无需暴露 API Key。
